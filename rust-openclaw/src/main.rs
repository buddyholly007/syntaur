...
pub fn resolve_data_dir() -> std::path::PathBuf {
    ...
    let legacy_dir = std::path::PathBuf::from(&home).join(".openclaw");
...
async fn run_bootstrap_admin(args: &[String]) {
    ...
    let db_path = PathBuf::from(format!("{}/index.db", std::path::Path::new(&std::env::var("HOME").unwrap_or_else(|_| "/home/sean".to_string())).join(if std::path::Path::new(&std::env::var("HOME").unwrap_or_else(|_| "/home/sean".to_string())).join(".syntaur").exists() { ".syntaur" } else { ".openclaw" }).to_string_lossy()));
...
async fn handle_propose_plan(
    ...
    if req.send_telegram {
        ...
        if let Ok(Some((plan, steps))) = state.plans.get(plan_id).await {
            let bot_token = "8706229027:AAGFvQBns2bTad1bthzgs7J4TrvAh3SB934";
            let chat_id = 8643081032_i64;
...
async fn main() {
    ...
    let tool_hooks_store = match tool_hooks::HookStore::open(
        ...
        "8706229027:AAGFvQBns2bTad1bthzgs7J4TrvAh3SB934".to_string(),
        8643081032,
        hooks_http,
    )
    .await
    ...
    let bind_addr = std::env::var("OPENCLAW_PORT")
        .ok()
