...
pub struct InvokeRequestBuilder<'a> {
    ...
    /// Interaction Model revision (required per Matter Core §10.4 in 1.1+).
    /// Tag 0xFF. 2026-04-25 patch: stock rs-matter omits this and Eve Energy
    /// rejects with InvalidAction. Setting always to 1 (Matter 1.0/1.1 IMR).
    #[tagval(0xff)]
...
impl<'a> InvokeRequestBuilder<'a> {
    ...
    pub const fn new(invoke_requests: &'a [CmdData<'a>], timed: bool) -> Self {
        Self {
            suppress_response: Some(false),  // 2026-04-25 patch: was None; spec
                                              // says optional but Eve requires
                                              // explicit value.
            timed_request: Some(timed),       // 2026-04-25 patch: was Option<bool>
                                              // omitted when false; spec says
                                              // REQUIRED bool.
            invoke_requests,
            interaction_model_revision: 1,    // Matter 1.1 IMR.
        }
    }
}
...
impl ImClient {
        ...
        F: FnMut(&InvokeResp<'_>) -> Result<(), Error>,
    {
        ...
        loop {
            ...
            let (more_chunks, suppress_response, cb_result) = {
                let rx = exchange.rx()?;
                let proto_op = rx.meta().proto_opcode;
                // 2026-04-25 patch: when responder sends a StatusResponse instead
                // of InvokeResponse (e.g. command rejected, fail-safe disarmed),
                // surface the IMStatusCode in the error so the caller knows WHY.
                // Stock rs-matter check_opcode just returns InvalidOpcode, hiding
                // the status. See vault/projects/path_c_plan.md 2026-04-25 entry.
                if proto_op == OpCode::StatusResponse as u8 {
                    let element = TLVElement::new(rx.payload());
                    eprintln!("=== PATCH FIRED: StatusResponse received ===");
                    if let Ok(sr) = StatusResp::from_tlv(&element) {
                        eprintln!("ImClient::invoke received StatusResponse: status={:?}", sr.status);
                    } else {
                        eprintln!("ImClient::invoke received StatusResponse (unparseable)");
                    }
                    return Err(ErrorCode::Invalid.into());
                }
                Self::check_opcode(proto_op, OpCode::InvokeResponse)?;
    ...
    fn check_opcode(received: u8, expected: OpCode) -> Result<(), Error> {
        if received != expected as u8 {
            // 2026-04-25 patch: when received opcode is StatusResponse (1),
            // dump the IMStatusCode meaning to stderr — bypasses LTO that
            // strips the StatusResponse-handling earlier in invoke().
            eprintln!("PATCH-CHECK-OPCODE: received={} expected_as_u8={}", received, expected as u8);
            error!(
...
impl ImClient {
    ...
    pub async fn invoke_single_cmd<'a>(
        ...
        // Check opcode and more_chunks before acknowledging
        {
            let rx = exchange.rx()?;
            let proto_op = rx.meta().proto_opcode;
            // 2026-04-25 patch: when responder sends StatusResponse instead
            // of InvokeResponse, surface the IMStatusCode so callers know WHY
            // the command was rejected. See vault/feedback/eve_att_order_quirk.md.
            if proto_op == OpCode::StatusResponse as u8 {
                let element = TLVElement::new(rx.payload());
                let status_str = match StatusResp::from_tlv(&element) {
                    Ok(sr) => format!("{:?}", sr.status),
                    Err(_) => "<unparseable>".to_string(),
                };
                eprintln!(
                    "PATCH-INVOKE-SINGLE: Eve sent StatusResponse instead of InvokeResponse, IMStatusCode={}",
                    status_str
                );
                error!(
                    "ImClient::invoke_single_cmd: peer responded with StatusResponse, IMStatusCode={}",
                    status_str
                );
                return Err(ErrorCode::Invalid.into());
            }
