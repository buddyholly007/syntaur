...
impl<C: Crypto> PaseInitiator<C> {
    ...
    async fn exchange_pbkdf_params(
        ...
        // Matter Core §5.1.4.5: salt is octet_string [16, 32] — accept any
        // length in that range (Eve Energy plug sends 16-byte salts).
        let salt_len = params.salt.0.len();
        if !(16..=SPAKE2P_VERIFIER_SALT_LEN).contains(&salt_len) {
            error!(
                "PBKDFParamResponse: invalid salt length {} (must be 16..=32)",
                salt_len
            );
            return Err(ErrorCode::Invalid.into());
        }
