...
impl<'a> SubjectDN<'a> {
    pub const fn rcac(fabric_id: u64, ca_id: u64) -> Self {
        Self { node_id: None, fabric_id: Some(fabric_id), cat_ids: &[], ca_id: Some(ca_id) }
    }
    pub const fn noc(fabric_id: u64, node_id: u64, cat_ids: &'a [u32]) -> Self {
        Self { node_id: Some(node_id), fabric_id: Some(fabric_id), cat_ids, ca_id: None }
    }
}
...
impl Validity {
    pub const fn new(not_before: u32, not_after: u32) -> Self {
