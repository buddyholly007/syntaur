//! Local PTY allocation — raw POSIX openpty + fork.
...
/// Spawn a local shell with a PTY.
pub fn spawn_pty(
    shell: &str,
    cols: u16,
    rows: u16,
...
pub async fn pty_reader_fd(fd: RawFd, tx: broadcast::Sender<Bytes>) {
    pty_reader(fd, tx).await;
}

pub async fn pty_writer_fd(fd: RawFd, rx: mpsc::Receiver<Bytes>) {
    pty_writer(fd, rx).await;
}

async fn pty_reader(fd: RawFd, tx: broadcast::Sender<Bytes>) {
...
async fn pty_writer(fd: RawFd, mut rx: mpsc::Receiver<Bytes>) {
