  ...
  for (const line of lines) {
    ...
    if (line.startsWith('**')) {
      ...
      if (match) {
        ...
        const text = match[3];
        const sourceClass = source.includes('phone') ? 'bg-blue-900/30 text-blue-400' :
                           source.includes('wearable') ? 'bg-purple-900/30 text-purple-400' :
                           source ? 'bg-gray-800 text-gray-400' : '';
        const sourceBadge = source ? `<span class="text-xs px-2 py-0.5 rounded-full ${sourceClass}">${source.replace(/[\[\]]/g,'')}</span>` : '';
        html += `<div class="bg-gray-900 border border-gray-800 rounded-xl p-4">
          ...
          <p class="text-gray-300 text-sm leading-relaxed">${esc(text)}</p>
        </div>`;
      }
    }
  }
...
async function doSearch() {
  ...
  for (const r of data.results) {
    html += `<div class="bg-gray-900 border border-gray-800 rounded-xl p-4">
      <button onclick="loadDate('${r.date}')" class="text-oc-500 hover:text-oc-400 text-sm font-medium mb-2 block">${formatDate(r.date)}</button>`;
    for (const m of r.matches.slice(0, 3)) {
      const highlighted = esc(m).replace(new RegExp(esc(q), 'gi'), '<mark class="bg-oc-900/50 text-oc-400 px-0.5 rounded">$&</mark>');
      html += `<p class="text-gray-400 text-sm leading-relaxed mb-1">${highlighted}</p>`;
    }
    if (r.count > 3) html += `<p class="text-gray-600 text-xs">...and ${r.count - 3} more</p>`;
    html += '</div>';
  }
...
async function loadDates() {
  ...
  if (data.dates && data.dates.length) {
    container.innerHTML = data.dates.slice(0, 14).map(d =>
      `<button onclick="loadDate('${d}')" class="text-xs bg-gray-900 border border-gray-800 hover:border-gray-700 rounded-lg px-3 py-1.5 text-gray-400 hover:text-white transition-colors">${d}</button>`
    ).join('');
  } else {
    container.innerHTML = '<span class="text-xs text-gray-600">No journal entries yet</span>';
  }
}
